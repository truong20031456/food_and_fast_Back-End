...
def not_found_exception(
    ...
    return create_http_exception(
        NotFoundError(message, error_code, details), status.HTTP_404_NOT_FOUND
    )
...
def conflict_exception(
    ...
    return create_http_exception(
        ConflictError(message, error_code, details), status.HTTP_409_CONFLICT
    )
...
def unauthorized_exception(
    ...
    return create_http_exception(
        UnauthorizedError(message, error_code, details), status.HTTP_401_UNAUTHORIZED
    )
...
def forbidden_exception(
    ...
    return create_http_exception(
        ForbiddenError(message, error_code, details), status.HTTP_403_FORBIDDEN
    )
