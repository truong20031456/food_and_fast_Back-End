...
async def get_current_user_dependency(token: Annotated[str, Depends(oauth2_scheme)], db: Annotated[AsyncSession, Depends(get_db)]):
...
async def get_me(current_user: Annotated[UserRead, Depends(get_current_user_dependency)]):
    return current_user
