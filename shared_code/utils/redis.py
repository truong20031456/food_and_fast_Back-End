...
class RedisManager:
    ...
    async def set_hash(
        self, key: str, mapping: dict, expire: Optional[int] = None
    ) -> bool:
        """Set hash fields"""
        try:
    ...
    async def get_hash(
        self, key: str, field: Optional[str] = None
...
def get_redis_manager(
    redis_url: str = None, service_name: str = "default"
) -> RedisManager:
    """Get current Redis manager"""
    global _redis_manager
    if redis_url and _redis_manager is None:
        ...
        _redis_manager = RedisManager(CustomSettings())
    elif _redis_manager is None:
    ...
    try:
        if _redis_manager:
            ...
            if asyncio.get_event_loop().is_running():
                asyncio.create_task(_redis_manager.close())
            else:
                asyncio.run(_redis_manager.close())
            _redis_manager = None
    except Exception:
        pass
