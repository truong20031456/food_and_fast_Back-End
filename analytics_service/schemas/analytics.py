...
class SalesSummaryResponse(BaseModel):
    ...
    total_orders: int = Field(..., description="Total number of orders")
    average_order_value: float = Field(..., description="Average order value")
    sales_by_period: List[SalesPeriodData] = Field(..., description="Sales breakdown by period")
    top_products: List[TopProductData] = Field(..., description="Top selling products")
    sales_by_category: List[CategorySalesData] = Field(..., description="Sales by category")
    generated_at: str = Field(..., description="Report generation timestamp")


class TopProductsResponse(BaseModel):
    """Top products response schema."""
    products: List[Dict[str, Any]] = Field(..., description="List of top selling products")
...
class DailySalesReportResponse(BaseModel):
    ...
    hourly_breakdown: List[Dict[str, Any]] = Field(..., description="Hourly sales breakdown")
    generated_at: str = Field(..., description="Report generation timestamp")