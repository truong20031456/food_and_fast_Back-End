...
def send_order_confirmation_email(order_id: int, user_email: str):
    ...
    try:
        ...
        print(
            f"Sending confirmation email for order {order.order_number} to {user_email}"
        )
...
def send_order_status_update(order_id: int, user_email: str, new_status: str):
    ...
    try:
        ...
        print(
            f"Sending status update email for order {order.order_number} to {user_email}: {new_status}"
        )
        ...
        return {
            "status": "success",
            "message": f"Status update email sent for order {order.order_number}",
        }
...
def cleanup_abandoned_carts():
    ...
    try:
        ...
        abandoned_carts = (
            db.query(Cart)
            .filter(and_(Cart.status == "active", Cart.updated_at < cutoff_time))
            .all()
        )
        ...
        preparing_orders = (
            ...
            .filter(
                and_(
                    ...
                    Order.confirmed_at
                    < now - timedelta(minutes=20),  # 20 min prep time
                )
            )
            .all()
        )
...
def notify_restaurant(order_id: int):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Restaurant notified about order {order.order_number}",
        }
