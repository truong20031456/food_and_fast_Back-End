...
class CartService:
    ...
    def get_or_create_cart(db: Session, user_id: Optional[int] = None, session_id: Optional[str] = None) -> Cart:
        ...
        if user_id:
            ...
            cart = db.query(Cart).filter(Cart.session_id == session_id, Cart.status == "active").first()
        else:
    ...
    @staticmethod
    def add_item_to_cart(db: Session, cart_id: int, item_data: CartItemCreate) -> CartItem:
        """Add item to cart"""
    ...
    @staticmethod
    def update_item_quantity(db: Session, cart_item_id: int, quantity: int) -> Optional[CartItem]:
